...
for q = 1:3
% Steady State Error
temp_ess = reshape(ess(:,:,n_kp(q),n_kv),N_f,N_tau);
% Plotting
...
zlim([0 15])
% Legends
legend({['PSM ',num2str(kp(n_kp(1))),' VSM = ',num2str(kv(n_kv))];
    ['PSM ',num2str(kp(n_kp(2))),' VSM = ',num2str(kv(n_kv))];
    ['PSM ',num2str(kp(n_kp(3))),' VSM = ',num2str(kv(n_kv))]},'Location','northeast')

savefig(gcf,'kp_kv.fig')
saveas(gcf,'kp_kv.jpg')
close(gcf)
